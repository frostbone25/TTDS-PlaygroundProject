...
RemovingAgentsWithPrefix = function(sceneObject, prefixString)
    local scene_agents     = SceneGetAgents(sceneObject)
...
RemovingAllLightingRigs = function(sceneObject)
    ...
    for i, agent_object in pairs(scene_agents) do
        ...
        if includeLightRigs then
            if (string.match)(agent_name, "lightrig_") then
                table.insert(rig_agents_names, agent_name)
            end
        end
    end
...
SetPropertyOnSpecificAgents = function(sceneObject, agentNamePrefix, propertyString, propertyValue)
    local scene_agents     = SceneGetAgents(sceneObject)
    local rig_agents_names = {}
...
SetPropertyKeyOnAllCameras = function(sceneObject, propertyKeyString, propertyValue)
    local scene_agents     = SceneGetAgents(sceneObject)
    local rig_agents_names = {}
    ...
    for x, rig_agent_name in pairs(rig_agents_names) do
        ...
        customSetProperty(agent_props, propertyKeyString, propertyValue)
    end
end
...
MigrateAgentsFromSceneToScene = function(originalScene, newScene, agentPrefix)
    local originalScene_agents = SceneGetAgents(originalScene)
    local agents_names = {}
    
    --fill out agents list
    for i, agent_object in pairs(originalScene_agents) do
        local agent_name = tostring(AgentGetName(agent_object))

        if (string.match)(agent_name, agentPrefix) then
            if not (string.match)(agent_name, ".scene") then
                table.insert(agents_names, agent_name)
            end
        end
    end
    
    --start creating agents in the new scene
    for x, agent_name in pairs(agents_names) do
        local originalSceneAgent = AgentFindInScene(agent_name, originalScene)
        local originalSceneAgent_props = AgentGetProperties(originalSceneAgent)
        
        local originalSceneAgent_pos = AgentGetPos(originalSceneAgent)
        local originalSceneAgent_rot = AgentGetRot(originalSceneAgent)
        local originalSceneAgent_worldPos = AgentGetWorldPos(originalSceneAgent)
        local originalSceneAgent_worldRot = AgentGetWorldRot(originalSceneAgent)
        
        local newAgent = AgentCreate(agent_name, originalSceneAgent_props, Vector(0, 0, 0), Vector(0, 0, 0), newScene, false, false)
       
        AgentSetWorldPos(newAgent, originalSceneAgent_worldPos)
        AgentSetWorldRot(newAgent, originalSceneAgent_worldRot)
        AgentSetPos(newAgent, originalSceneAgent_pos)
        AgentSetRot(newAgent, originalSceneAgent_rot)
    end
end

--prints an entire scene and its contents to a text file
PrintSceneLightsToTXT = function(SceneObject, txtName)
    local main_txt_file                 = io.open(txtName, "a")
    local scene_agents                  = SceneGetAgents(SceneObject)
    local print_agent_transformation    = true
    local print_agent_properties        = true
    
    local lightNamePrefix = "light_"
    
    --being looping through the list of agents gathered from the scene
    for i, agent_object in pairs(scene_agents) do
        local agent_name = tostring(AgentGetName(agent_object))
        
        if (string.match)(agent_name, lightNamePrefix) then
            --start writing the agent information to the file
            main_txt_file:write(" ", "\n")
            main_txt_file:write(i .. ". " .. agent_name, "\n")
            ...
            if print_agent_transformation then
                ...
                main_txt_file:write("Position: " .. agent_pos, "\n")
                main_txt_file:write("Rotation: " .. agent_rot, "\n")
                main_txt_file:write("World Position: " .. agent_pos_world, "\n")
                main_txt_file:write("World Rotation: " .. agent_rot_world, "\n")
            end
            ...
            if agent_properties and print_agent_properties then
                --write a quick header to seperate
                main_txt_file:write("--- Agent Properties ---", "\n")
            
                local propName = ""
                local value = nil
                local valueType = nil
                
                propName = "EnvLight - Type"
                if PropertyExists(agent_properties, propName) then
                    value = PropertyGet(agent_properties, propName)
                    valueType = TypeName(PropertyGet(agent_properties, propName))
                    
                    main_txt_file:write("Name: " .. propName, "\n")
                    main_txt_file:write("Value: " .. tostring(value), "\n")
                    main_txt_file:write("Value Type: " .. tostring(valueType), "\n")
                end
                
                propName = "EnvLight - Intensity"
                if PropertyExists(agent_properties, propName) then
                    value = PropertyGet(agent_properties, propName)
                    valueType = TypeName(PropertyGet(agent_properties, propName))
                ...
                propName = "EnvLight - Distance Falloff"
                if PropertyExists(agent_properties, propName) then
                    value = PropertyGet(agent_properties, propName)
                    valueType = TypeName(PropertyGet(agent_properties, propName))
                ...
                propName = "EnvLight - Spot Angle Inner"
                if PropertyExists(agent_properties, propName) then
                    value = PropertyGet(agent_properties, propName)
                    valueType = TypeName(PropertyGet(agent_properties, propName))
                ...
                propName = "EnvLight - Spot Angle Outer"
                if PropertyExists(agent_properties, propName) then
                    value = PropertyGet(agent_properties, propName)
                    valueType = TypeName(PropertyGet(agent_properties, propName))
                ...
                propName = "EnvLight - Color"
                if PropertyExists(agent_properties, propName) then
                    value = PropertyGet(agent_properties, propName)
                    valueType = TypeName(PropertyGet(agent_properties, propName))
                    
                    main_txt_file:write("Name: " .. propName, "\n")
                    main_txt_file:write("Value Table", "\n")
                    main_txt_file:write(tprint(value), "\n")
                end
                
                propName = "EnvLight - Local Position"
                if PropertyExists(agent_properties, propName) then
                    value = PropertyGet(agent_properties, propName)
                    valueType = TypeName(PropertyGet(agent_properties, propName))
                ...
                propName = "EnvLight - Local Rotation"
                if PropertyExists(agent_properties, propName) then
                    value = PropertyGet(agent_properties, propName)
                    valueType = TypeName(PropertyGet(agent_properties, propName))
            ...
            --write a header to indicate the end of the agent properties information
            main_txt_file:write("---Agent Properties END ---", "\n")
        end
    end
